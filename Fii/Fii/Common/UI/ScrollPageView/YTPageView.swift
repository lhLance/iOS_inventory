...
class YTPageView: UIView {
    ...
    init(frame: CGRect, titles: [String], childVCs: [UIViewController], parentVC: UIViewController, titleStyle: YTPageStyle) {
        self.titles = titles
...
//        let titleFrame = CGRect(x: 0, y: 64, width: bounds.width, height: titleStyle.titleViewHeight)
        ...
        let titleFrame = CGRect(x: 0, y: kNavigationBar + kStatusBarH, width: bounds.width, height: titleStyle.titleViewHeight)

        let  titleView = YTPageTitleView(frame: titleFrame, titles: titles, style: titleStyle)
//        titleView.backgroundColor = UIColor.init(hexString: "##FF0022")
        titleView.backgroundColor = UIColor.white
        addSubview(titleView)
        //添加contentView
        let contentFrame = CGRect(x:0, y: titleFrame.maxY, width: bounds.size.width, height: bounds.size.height - contentH )
        let   contentView = YTPageContenvView(frame: contentFrame, childVs: childVCs, parentVc: parentVC)
