...
protocol YTPageTitleViewDelegate: class {
    //不用参数时，可以在参数前添加下划线
...
//MARK:- 遵循YTContentViewDelegate
extension YTPageTitleView:YTPageContenvViewDelegate{
    ...
    func contentView(_ contentView: YTPageContenvView, sourceIndex: Int, targetIndex: Int, progress: CGFloat) {
        ...
        if style.isNeedScale {
            ...
            sourceLabel.transform = CGAffineTransform(scaleX: style.maxScale - detalScale * progress,
                                                      y: style.maxScale - detalScale * progress)
            targetLabel.transform = CGAffineTransform(scaleX: 1.0 + detalScale * progress,
                                                      y: 1.0 + detalScale * progress)
        }
        //5.coverView渐变
        if style.isShowCoverView {
            UIView.animate(withDuration: 0.3, animations: {
